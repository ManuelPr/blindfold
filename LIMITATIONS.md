...
- **The compute child has full `__builtins__`, so the filesystem is readable.** `open()` and `import` both work; `os.listdir(".")` from inside a compute call returns the working directory. The child is `python -I` with a stripped environment, which removes `site-packages` and inherited variables — not file access.
  ...
  **Fix: cheap and partial.** Replace `__builtins__` in the exec scope with a small allow-list (`len`, `sum`, `sorted`, `min`, `max`, `abs`, `round`, the basic types) so `open` and `__import__` are simply absent. This blocks the direct route and does not restrict any legitimate computation. It is **not** airtight — escaping a restricted namespace through Python's object graph is a known exercise — so treat it as raising the cost, not as isolation.

- **Network access from compute code is open on Linux and macOS.** On Windows it currently fails, but only because the stripped environment omits `SystemRoot` and Windows cannot initialize sockets without it. That is an accident of the environment cleanup, not a designed control, and it does not reproduce on other platforms. The README previously claimed the sandbox had no network; it does.

  **Fix: only properly at OS level.** Blocking `socket`/`urllib` imports (as above) removes the easy path. Actually denying the network means a container with networking disabled, or platform-specific namespace work. This is the strongest argument for the Docker sandbox — and also the reason it is not urgent: the value can already leave through the exception channel and the one-bit oracle, neither of which a container closes.
...
- **Unsupported path syntax fails silently.** `$..salary` does not raise; it matches nothing, so the field is never tokenized and its value reaches the model in cleartext. Same for a typo in a key name. Only a missing `$` prefix raises. This is the same forgiving behavior that makes defensive declaration cheap, and it cuts the other way: a config that looks right can protect nothing.

  **Fix:** feasible — reject syntax the dialect cannot honor at config load, which separates "this path did not match this response" (fine, intended) from "this path can never match anything" (a bug in your config). Until then, assert on tokenized output in your integration tests; see [`tests/integration/test_proxy_forwarding.py`](tests/integration/test_proxy_forwarding.py) for the pattern.
